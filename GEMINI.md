...
- **`deploy.sh`:** STIG-compliant deployment wrapper. Validates image pinning, resource limits, and network isolation locally, then syncs and deploys stacks to the remote node via SSH. Falls back to local deployment if no remote is configured. Supports `up`, `down`, `ps`, `logs`, `restart`, `check`, `redeploy`, and `full-check` commands.
...
- **`scripts/clean-node.sh`:** Completely wipes the remote node (containers, volumes, networks, data). Accepts `--yes` (skip confirmation) and `--images` (also prune container images). Respects `SSH_PORT` and `REMOTE_PROJECT_DIR` from `.env`.
